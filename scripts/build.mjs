...
function stylesheet(name, css) {
 css=transformSync(css,{loader:'css',minify:true,target:'es2022'}).code;
...
// Discover every static dependency up front; dynamic counter code stays on demand.
function modulePreloads(entry) {
 const seen=new Set();
 function visit(file) {
  if(seen.has(file))return;
  seen.add(file);
  for(const dependency of bundle.metafile.outputs[file].imports) {
   if(dependency.kind==='import-statement'&&!dependency.external)visit(dependency.path);
  }
 }
 visit(Object.keys(bundle.metafile.outputs).find(file=>bundle.metafile.outputs[file].entryPoint===entry));
 return [...seen].map(file=>`<link rel="modulepreload" href="/${path.relative(out,path.resolve(file)).split(path.sep).join('/')}">`).join('');
}
...
for(const [index,page] of pages.entries()){
 ...
 const html=head(page.title,url)+`<body class="standards"><a class="skip" href="#main">Skip to content</a><aside class="sidebar"><div class="brand"><h1><a href="/">Tomorrow We Are—Standards</a></h1><p>Working reference · ${version}</p></div><button class="docs-menu-toggle" type="button" aria-expanded="false" aria-controls="docs-menu">Browse sections <span aria-hidden="true">+</span></button><div class="docs-menu" id="docs-menu"><nav class="category-nav" aria-label="Documentation">${nav(url)}</nav><footer class="sidebar-foot">${themeMarkup}<p><a href="https://github.com/AnushGopalakrishnan/twa-standards">Source on GitHub ↗</a></p><p>Copyright © Tomorrow We Are 2026</p></footer></div></aside><main class="content" id="main" tabindex="-1"><header class="doc-heading"><p class="eyebrow">${page.group} / ${String(index+1).padStart(2,'0')}</p><h1>${page.title}</h1><p class="purpose">${page.purpose}</p></header>${(await Promise.all(page.examples.map(demo))).join('')}<div class="doc-body">${page.body.replaceAll('RELEASE_COMMIT',revision)}</div><nav class="doc-pagination" aria-label="Next and previous pages">${index>0?`<a href="${route(pages[index-1])}"><small>Previous</small>${pages[index-1].title}</a>`:''}${index<pages.length-1?`<a href="${route(pages[index+1])}"><small>Next</small>${pages[index+1].title} ↗</a>`:''}</nav></main>${page.examples.includes('signup')?signup:''}<script type="module" src="${entryURL('site/client.js')}"></script></body></html>`;
...
write('examples/gallery/index.html',head('Gallery demonstration','/examples/gallery/',patternsCSS,'site/gallery-demo.js')+`<body><a class="skip" href="#gallery">Skip to gallery</a><aside class="sidebar"><div class="sidebar-head"><div class="brand"><h1 class="wordmark">Tomorrow We Are—Standards</h1><p data-demo-title>Gallery demonstration · 6</p></div></div><div class="intro"><p data-demo-instruction>Six synthetic references for trying the shared gallery and viewer.</p><p><a data-demo-return href="/patterns/gallery/">← Back to Gallery cards and grid</a></p><div class="newsletter"><p class="newsletter-copy">Try the signup interaction.</p><div class="newsletter-actions"><button class="button button--primary newsletter-open" type="button">Sign up</button></div></div></div><nav class="category-nav" aria-label="Example categories"></nav><footer class="sidebar-foot">${themeMarkup}<p>Synthetic data. No email is sent or saved.</p><p>Captured at 1440 × 900</p></footer></aside><main class="content" id="gallery"></main>${signup}${read('src/patterns/viewer.html')}<script>window.DEMO_PLACEHOLDER='data:image/webp;base64,${placeholder}';</script><script type="module" src="${entryURL('site/gallery-demo.js')}"></script></body></html>`);
...
write('404.html',head('Page not found','/404/',standardsCSS,null)+'<body class="standards"><main style="padding:48px"><h1>Page not found</h1><p><a href="/">Return to the foundations overview ↗</a></p></main></body></html>');
