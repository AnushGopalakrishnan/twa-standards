...
The loopback preview at port 3005 expires after one hour. `npm run build` generates the documentation and synthetic gallery into ignored `dist/`. `npm test` checks all documentation pages at desktop/mobile sizes, examples, focus restoration, reduced motion and the absence of unexpected external requests. Presentation checks cover all 18 example pages, expanded code, copy-button states, inert references, theme sync, typography, divider width, retry framing and contextual demo returns. The only permitted external asset is the licensed webfont. Navigation checks cover partial page updates, history, scroll, focus, prefetching, cleanup and full-page fallbacks. `npm run test:deploy` verifies the production guard.

Documentation uses 14px body copy, 16px section headings and 12px supporting labels; code keeps 13px monospace. Copy buttons show confirmation inside the button, then return to their default label.

Edit `site/pages.mjs` for documentation, `site/examples.mjs` for working examples, and `site/site.css` for documentation layout. The typography font is explicitly downloaded even when a matching local face is installed. Edit `site/presentation.mjs` for named states, anatomy and minimal snippets. `scripts/render-demo.mjs` formats HTML and JavaScript and applies Highlight.js syntax highlighting at build time (HTML/JavaScript grammars and bundled GitHub light/dark styles; no browser highlighter), with separate copyable panels and expandable complete demo source. Only example setup functions enter the runtime bundle; reference markup and code samples stay in generated HTML. Pattern pages include their shared templates and full demo source. `scripts/static-references.mjs` renders non-interactive reference markup from shared templates and the gallery renderer using a build-only DOM. Isolated, lazy `srcdoc` frames load the shared stylesheet, contain no scripts, and mark their content inert. The documentation adjusts their display scale and theme; light/dark toggle comparisons retain their labelled palette. No screenshots of components are stored. The gallery uses six synthetic local SVGs; signup callbacks simulate responses without sending or retaining email addresses. `_headers` limits network requests, prevents native form submission, and opts out of Cloudflare HTML transformations (including automatic analytics injection).

## Documentation navigation

Documentation links keep the sidebar mounted and replace only the main content and page-specific dialog. URLs, titles, canonical metadata and the active link update together. New pages receive keyboard focus. Mobile documentation navigation is collapsed behind Browse sections, closes on selection or Escape, and leaves the first preview visible near the top. Without JavaScript, all navigation links remain visible. Back/forward restores each history entry's scroll position. Page examples start fresh on a return visit and release their global listeners when leaving.
